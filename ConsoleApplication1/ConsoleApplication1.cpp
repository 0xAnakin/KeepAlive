...
{   
    ...
    int min = SEC * 60; // 1 minutes
    int max = SEC * 60 * 5;  // 5 minutes

    if (argc > 1) 
    {
        if (is_valid_integer_integer(argv[1]))
        {   
            min = (SEC * 60) * atoi(argv[1]);
        }
        ...
        if (argc > 2)
        {
            ...
            {
                int new_max = (SEC * 60) * atoi(argv[2]);
                // in case new_max is not bigger than min, adjust it
                max = new_max > min ? new_max : min + (SEC * 60);
    ...
    std::cout << "interval range: " << ((min / 1000) / 60) << "mins - " << ((max / 1000) / 60) << "mins" << std::endl;
    ...
    while (true) {
        ...
        std::cout << "Sleeping for " << ((interval/1000)/60)  << "mins" << std::endl;
